...
`system.access` (audit logs, table/column lineage) is auto-managed by Databricks — confirmed via `databricks system-schemas list <metastore_id>`, which reports `state: MANAGED`, the same bucket as `billing`/`compute`/etc. **Don't** declare it with a `databricks_system_schema` resource: doing so fails every time with "Provider produced inconsistent result after apply — Root object was present, but now absent", since the provider doesn't expect to manage the lifecycle of an already-auto-enabled schema. On a brand-new metastore it still takes a short while to actually materialize, though — `databricks_grants.system_access` (granting `sp-data-platform` `USE_SCHEMA`/`SELECT`, needed by `databricks-platform-governance`'s `lineage_cache_refresh` job to read `system.access.table_lineage`/`column_lineage`) hit "Schema 'system.access' does not exist" seconds after `databricks_metastore_assignment.this` completed on a fresh metastore. `time_sleep.wait_for_system_schemas` (60s) sits between them to buy that propagation window.
